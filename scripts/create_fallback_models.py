#!/usr/bin/env python3
"""
Script pour créer des modèles TFLite de remplacement simples (CPU)
Ces modèles sont des placeholders pour tester le système sans EdgeTPU.

⚠️ ATTENTION : Ces modèles ne détectent PAS vraiment les pièces !
Ils retournent des résultats aléatoires pour permettre de tester le système.

Pour une vraie détection, vous devez reconvertir les modèles originaux.
"""

import os
import sys
import numpy as np

try:
    import tensorflow as tf
except ImportError:
    print("❌ TensorFlow n'est pas installé.")
    print("Installez-le avec: pip install tensorflow")
    sys.exit(1)

print("="*70)
print("Création de modèles TFLite de remplacement (CPU)")
print("="*70)
print()

# Chemins
script_dir = os.path.dirname(os.path.abspath(__file__))
project_dir = os.path.dirname(script_dir)
models_dir = os.path.join(project_dir, 'reachy_tictactoe', 'models')

print(f"📁 Répertoire des modèles: {models_dir}")
print()

# Vérifier que le répertoire existe
if not os.path.exists(models_dir):
    print(f"❌ Le répertoire {models_dir} n'existe pas")
    sys.exit(1)


def create_simple_classifier_model(num_classes, input_shape=(224, 224, 3)):
    """
    Crée un modèle de classification simple
    
    Args:
        num_classes: Nombre de classes
        input_shape: Forme de l'entrée (hauteur, largeur, canaux)
    
    Returns:
        tf.keras.Model
    """
    model = tf.keras.Sequential([
        tf.keras.layers.InputLayer(input_shape=input_shape),
        tf.keras.layers.Conv2D(8, 3, activation='relu'),
        tf.keras.layers.MaxPooling2D(2),
        tf.keras.layers.Conv2D(16, 3, activation='relu'),
        tf.keras.layers.MaxPooling2D(2),
        tf.keras.layers.GlobalAveragePooling2D(),
        tf.keras.layers.Dense(num_classes, activation='softmax')
    ])
    return model


def convert_to_tflite(model, output_path):
    """
    Convertit un modèle Keras en TFLite
    
    Args:
        model: Modèle Keras
        output_path: Chemin de sortie du fichier .tflite
    """
    converter = tf.lite.TFLiteConverter.from_keras_model(model)
    
    # Optimisations pour CPU
    converter.optimizations = [tf.lite.Optimize.DEFAULT]
    converter.target_spec.supported_ops = [
        tf.lite.OpsSet.TFLITE_BUILTINS,  # Opérations TFLite standard
    ]
    
    # Convertir
    tflite_model = converter.convert()
    
    # Sauvegarder
    with open(output_path, 'wb') as f:
        f.write(tflite_model)
    
    print(f"✓ Modèle créé: {os.path.basename(output_path)}")
    print(f"  Taille: {len(tflite_model) / 1024:.1f} KB")


# 1. Créer le modèle pour la classification des cases (3 classes: vide, cube, cylindre)
print("1️⃣  Création du modèle de classification des cases...")
print("   Classes: 0=vide, 1=cube, 2=cylindre")

boxes_model = create_simple_classifier_model(num_classes=3, input_shape=(224, 224, 3))

# Sauvegarder l'ancien modèle
old_boxes_path = os.path.join(models_dir, 'ttt-boxes.tflite')
if os.path.exists(old_boxes_path):
    backup_path = os.path.join(models_dir, 'ttt-boxes-edgetpu.tflite.backup')
    if not os.path.exists(backup_path):
        os.rename(old_boxes_path, backup_path)
        print(f"   ↪ Ancien modèle sauvegardé: {os.path.basename(backup_path)}")

# Convertir et sauvegarder
convert_to_tflite(boxes_model, old_boxes_path)
print()


# 2. Créer le modèle pour la validation du plateau (2 classes: valid, invalid)
print("2️⃣  Création du modèle de validation du plateau...")
print("   Classes: 0=invalid, 1=valid")

valid_model = create_simple_classifier_model(num_classes=2, input_shape=(224, 224, 3))

# Sauvegarder l'ancien modèle
old_valid_path = os.path.join(models_dir, 'ttt-valid-board.tflite')
if os.path.exists(old_valid_path):
    backup_path = os.path.join(models_dir, 'ttt-valid-board-edgetpu.tflite.backup')
    if not os.path.exists(backup_path):
        os.rename(old_valid_path, backup_path)
        print(f"   ↪ Ancien modèle sauvegardé: {os.path.basename(backup_path)}")

# Convertir et sauvegarder
convert_to_tflite(valid_model, old_valid_path)
print()


# 3. Vérifier les fichiers de labels
print("3️⃣  Vérification des fichiers de labels...")

boxes_labels_path = os.path.join(models_dir, 'ttt-boxes.txt')
if os.path.exists(boxes_labels_path):
    print(f"   ✓ {os.path.basename(boxes_labels_path)} existe")
else:
    # Créer le fichier de labels
    with open(boxes_labels_path, 'w') as f:
        f.write("0 empty\n")
        f.write("1 cube\n")
        f.write("2 cylinder\n")
    print(f"   ✓ {os.path.basename(boxes_labels_path)} créé")

valid_labels_path = os.path.join(models_dir, 'ttt-valid-board.txt')
if os.path.exists(valid_labels_path):
    print(f"   ✓ {os.path.basename(valid_labels_path)} existe")
else:
    # Créer le fichier de labels
    with open(valid_labels_path, 'w') as f:
        f.write("0 invalid\n")
        f.write("1 valid\n")
    print(f"   ✓ {os.path.basename(valid_labels_path)} créé")

print()


# 4. Tester les modèles
print("4️⃣  Test des modèles...")

try:
    import tflite_runtime.interpreter as tflite
except ImportError:
    import tensorflow.lite as tflite

# Tester le modèle des cases
try:
    interpreter = tflite.Interpreter(old_boxes_path)
    interpreter.allocate_tensors()
    print("   ✓ ttt-boxes.tflite fonctionne (CPU)")
except Exception as e:
    print(f"   ❌ ttt-boxes.tflite: {e}")

# Tester le modèle de validation
try:
    interpreter = tflite.Interpreter(old_valid_path)
    interpreter.allocate_tensors()
    print("   ✓ ttt-valid-board.tflite fonctionne (CPU)")
except Exception as e:
    print(f"   ❌ ttt-valid-board.tflite: {e}")

print()
print("="*70)
print("✅ Modèles de remplacement créés avec succès !")
print("="*70)
print()
print("⚠️  IMPORTANT:")
print("   Ces modèles sont des PLACEHOLDERS pour tester le système.")
print("   Ils ne détectent PAS vraiment les pièces du jeu.")
print()
print("   Pour une vraie détection, vous devez:")
print("   1. Reconvertir les modèles originaux (voir EDGE_TPU_CONVERSION.md)")
print("   2. Ou entraîner de nouveaux modèles avec vos propres données")
print("   3. Ou ajouter un accélérateur EdgeTPU USB")
print()
print("Vous pouvez maintenant tester le système avec:")
print("   python -m reachy_tictactoe.game_launcher --log-file /tmp/tictactoe")
print()

