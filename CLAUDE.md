...
python -m reachy_tictactoe.webapp --host localhost --port 8080
```
Boutons : « Lancer une partie », « Tester les mouvements » (parcourt les 9 cases à vide), « Calibrer » (à venir). ⚠️ Le port 8000 est déjà pris sur le NUC.
...
- **`display_board()` est une fenêtre de CONFORT, jamais bloquante** — `cv.imshow` échoue sans écran (serveur en SSH, opencv-headless) et n'est pas supporté hors thread principal, or l'interface web joue les parties **dans un thread de fond**. L'affichage se désactive donc tout seul au premier échec (`_display_available`), avec un seul avertissement. Ne pas remettre d'appel `cv.imshow` non protégé.
